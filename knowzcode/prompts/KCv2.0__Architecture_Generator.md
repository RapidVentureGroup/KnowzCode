# KnowzCode v2.0 - Flowchart Generation Guide

**Version:** 2.0
**Date:** 2025-06-10
**Based on:** Architecture generation principles from previous versions
**Target Audience:** KnowzCode Orchestrator (to guide the AI Agent)
**Purpose:** This document guides the AI Agent to generate or assist in updating the Mermaid flowchart syntax for the `./knowzcode_architecture.md` file. This flowchart is a critical visual representation of the application's architecture, component interactions, and dependencies within the KnowzCode v2.0 system.

---

**When this Guide is Used by KnowzCode Orchestrator:**

1.  **Initial Flowchart Creation:** During "Phase 1: Define, Visualize, Link" of KnowzCode v2.0, to create the first version of `knowzcode_architecture.md` based on `knowzcode_project.md` and/or existing code analysis.
...
3.  **Flowchart Consistency Review (Assistance):** To help identify or draft syntax for changes needed after a node's implementation reveals architectural discrepancies (as part of the "Check Flowchart Consistency" step in `knowzcode_loop.md`), especially if changes are complex and flagged for Orchestrator/user review.

**Input Provided by KnowzCode Orchestrator (will vary by use case):**

*   `ProjectOverviewContext`: Key architectural descriptions, main components, and features from `knowzcode_project.md`.
*   `ExistingFlowchartContent` (If updating): The current content of `knowzcode_architecture.md`.
...
*   `KnowzCodeNodeIDConvention`: The established convention for NodeIDs (e.g., `FEAT_Name`, `API_Name`, as potentially defined in `knowzcode_project.md` or the legend of an existing flowchart).

**Contextual Files for Agent's Analysis (Use your tools like `str_replace_editor view` and `search_filesystem`):**

*   `./knowzcode_project.md`: For high-level architecture, components, and features.
*   `./knowzcode_tracker.md`: To see existing NodeIDs and their labels (though the flowchart is the source of truth for architectural NodeIDs, and the status map is primary for spec links).
...
2.  **Node Definition & NodeIDs:**
    ...
    *   Assign a **unique, descriptive `NodeID`** to each implementable component represented in the flowchart. Adhere to the established `KnowzCodeNodeIDConvention` if one exists (check existing legend or `knowzcode_project.md`).
...
3.  **Spec Links (in Flowchart - Deprioritized):**
    *   The `./knowzcode_tracker.md` is the **primary artifact** for managing direct links to `specs/[NodeID].md` files.
...
*   Your final output for this task is **ONLY** the complete Mermaid flowchart syntax, ready to be saved as or inserted into `knowzcode/knowzcode_architecture.md`.
*   Start with `graph TD` (or `graph LR`) and end after the last element of the diagram, including the legend.
*   Do not include any other conversational text, explanations, or Markdown formatting around the Mermaid block itself (unless the entire file content of `knowzcode/knowzcode_architecture.md` is requested, in which case it should be pure Markdown with the Mermaid block as its primary content).
