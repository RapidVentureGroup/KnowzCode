# KnowzCode v2.0: Start Work Session

## Your Mission: Orient, Synchronize, and Collaborate on Next Goal

Hello, Agent. We are initiating a new work session under the **KnowzCode v2.0 methodology**. Your primary directive is to operate as an autonomous developer, guided by the Orchestrator and adhering strictly to the KnowzCode framework.

> **Automation Path:** Kick off the loop with `/kc-step phase=1A` after any strategic planning via `/kc-plan`. This command automatically loads the `impact-analyst` subagent with `load-core-context` and `tracker-scan` skills.

**Todo Discipline:** Keep the WorkGroup's task list inside `knowzcode/workgroups/<WorkGroupID>.md`. Every bullet should begin with `KnowzCode:` so the automation can surface it after each command.

This prompt will guide you through your "boot-up" sequence: orienting to the KnowzCode protocol, synchronizing with the project's current state, and **collaborating with the Orchestrator on what to build next**.

---

### 1. Foundational Protocol Review

**Your single source of truth for the operational cycle is `knowzcode_loop.md`.** This document contains your core principles, file interaction protocols, the step-by-step main operational loop, and all other procedural rules. You must adhere to it meticulously.

### 2. Core Project Artifacts

Your work will be informed by the following core artifacts. You are expected to read and interact with them as defined in `knowzcode_loop.md`:
*   **Project Blueprint:** `knowzcode_project.md` (Goals, Tech Stack, Standards)
*   **System Architecture:** `knowzcode_architecture.md` (Component Flowchart)
*   **Live Status Map:** `knowzcode_tracker.md` (Node Status & Dependencies)
*   **Component Blueprints:** The `specs/` directory
*   **Project History:** `knowzcode_log.md` (Operational Log & Quality Criteria)
...
*   **Project Overview Maintenance:** Significant changes update `knowzcode_project.md` during Loop 3.
...
2.  **Review Recent Activity:** Check the last 3-5 entries in the "Operational Log" section of `knowzcode_log.md` to understand the most recent project activities.
3.  **Check Project Status:** Review `knowzcode_tracker.md` to assess the current overall progress and the status of all nodes.
...
>
> **Ready to begin the formal KnowzCode process?**
>
> If yes, please use the prompt `KCv2.0__[LOOP_1A]__Propose_Change_Set.md` with:
