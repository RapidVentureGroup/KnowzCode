# KnowzCode v2.0: Pre-Flight Feature Analysis
...
> **Automation Path:** Launch `/kc-plan plan=pre-flight` to engage the `planning-strategist` subagent with `load-core-context` before filling in this prompt.
...
*   **Architectural Fit:** [Briefly state how this new feature fits into the existing patterns defined in `knowzcode_project.md`.]
...
**Next Command:** To start development, use **`KnowzCode v2.0: Start Work Session`** and provide the exact `PrimaryGoal` from this analysis. The agent will then use this report as the primary input for proposing the formal Change Set in `[LOOP_1A]`.
