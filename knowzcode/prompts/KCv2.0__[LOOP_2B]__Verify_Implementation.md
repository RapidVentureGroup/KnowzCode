# KnowzCode v2.0: [LOOP 2B] Verify Implementation Completeness
...
> **Automation Path:** Trigger `/kc-step phase=2B workgroup_id=<ID>` or `/kc-audit audit=implementation workgroup_id=<ID>` to run the `arc-auditor` subagent in read-only mode. It utilizes `spec-quality-check`, `tracker-scan`, and `log-entry-builder` skills to compute the completion percentage.
