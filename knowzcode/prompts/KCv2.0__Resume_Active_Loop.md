...
Work is already in progress. Your mission is to understand KnowzCode, reconstruct the active work context, determine the exact loop position, and be ready to continue immediately.

---

### PHASE 1: KnowzCode System Orientation

**KnowzCode v2.0** is a systematic development methodology that transforms chaotic AI coding into disciplined engineering.
...
**Architecture**: The `knowzcode_architecture.md` file contains a Mermaid diagram showing all NodeIDs and their connections. This is your system map.
...
**Critical Files**:
- `knowzcode_loop.md` - Your operational manual (read sections 1-3)
- `environment_context.md` - Platform-specific commands (use exactly)
- `knowzcode_project.md` - Tech stack and standards
- `knowzcode_tracker.md` - Status dashboard for all NodeIDs
- `knowzcode_log.md` - Project history and decisions
...
```bash
grep -E "\[WIP\].*feat-|fix-|refactor-|issue-" knowzcode/knowzcode_tracker.md
```
...
```bash
grep "\[WIP\]" knowzcode/knowzcode_tracker.md | awk '{print $3}' | sort -u
```
...
#### Original Goal
```bash
grep -A 5 -B 2 "$WORKGROUPID" knowzcode/knowzcode_log.md | head -20
```
...
If awaiting **Loop 1B**:
> "To continue, please provide: `KCv2.0__[LOOP_1B]__Draft_Specs.md` with the confirmed Change Set"

If awaiting **Loop 2A**:
> "To continue, please provide: `KCv2.0__[LOOP_2A]__Implement_Change_Set.md` with WorkGroupID: `[id]`"

If awaiting **Loop 2B**:
> "To continue, please provide: `KCv2.0__[LOOP_2B]__Verify_Implementation.md` with WorkGroupID: `[id]`"

If awaiting **Loop 3**:
> "To continue, please provide: `KCv2.0__[LOOP_3]__Finalize_And_Commit.md` with WorkGroupID: `[id]`"
