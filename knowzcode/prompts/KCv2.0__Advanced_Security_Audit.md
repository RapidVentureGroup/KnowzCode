# KnowzCode v2.0: Advanced Security Audit

> **Automation Path:** Trigger `/kc-audit audit=security` to delegate to the `security-officer` subagent. It loads `load-core-context`, `spec-quality-check`, and `tracker-scan` skills automatically.

## Your Mission
You are to perform a comprehensive, context-aware security audit of the entire application. Go beyond simple dependency scanning. Your mission is to analyze the application's architecture, data flows, and business logic to identify potential vulnerabilities based on the **OWASP Top 10** standard. The output will be a prioritized, actionable report that integrates with the KnowzCode workflow.
...
*   **Critical Issues (Address Immediately):**
    *   For `API_User`: The vulnerability requires a code change. **Next Command:** Use `KCv2.0__Handle_Critical_Issue.txt` to triage and fix this bug.
*   **High-Severity Issues (Address This Cycle):**
    *   For `Config`: This is a small, localized fix. **Next Command:** Use `KCv2.0__Execute_Micro_Fix.txt` to move the secret.
*   **Medium/Low-Severity & Technical Debt:**
    *   For `Dependencies`: This is a dependency update. **Next Command:** Schedule this work by creating a `REFACTOR_Dependencies` task in `knowzcode_tracker.md`.
