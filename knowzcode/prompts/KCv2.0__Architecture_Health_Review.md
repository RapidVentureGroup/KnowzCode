# KnowzCode v2.0: Architecture Health Review

> **Automation Path:** Run `/kc-audit audit=architecture` to engage the `architecture-reviewer` subagent with `load-core-context` and `architecture-diff` skills.

## Your Mission
You are to perform a systematic, in-depth review of the project's overall architectural health. Your goal is to identify architectural drift, technical debt, performance bottlenecks, and security risks. The output of this review will be a comprehensive report and a prioritized, actionable plan for improvement that feeds directly back into the KnowzCode development workflow.
...
*This phase requires manual analysis and comparison of the KnowzCode artifacts against the codebase.*

1.  **Architecture vs. Reality Alignment:**
    *   Compare `knowzcode_architecture.md` with the actual implementation.
...
*   Prepare a comprehensive `ArchitectureReview` log entry for `knowzcode/knowzcode_log.md`.
    ```markdown
    ---
...
### Phase 4: Action Plan
*This is the most critical output. Provide a clear, prioritized plan that links directly to KnowzCode protocols.*
...
1.  **Immediate Fixes (Micro-Fix Candidates):**
    ...
    *   **Next Command:** "For these items, use `KCv2.0__Execute_Micro_Fix.txt`."
2.  **Technical Debt Refactoring (Refactor Node Candidates):**
    ...
    *   **Next Command:** "For each of these nodes, a `REFACTOR_[NodeID]` task should be created in `knowzcode_tracker.md`. The work can then be executed using `KCv2.0__Refactor_Node.txt`."
3.  **Architectural Corrections (KnowzCode Loop Candidates):**
    *   List any major architectural corrections needed (e.g., correcting drift, splitting a God object into new nodes).
    *   **Next Command:** "This work requires a full planning cycle. Use `KCv2.0__Major_Mid_Project_Feature_Addition.txt` to formally plan the architectural changes and update the tracker."
