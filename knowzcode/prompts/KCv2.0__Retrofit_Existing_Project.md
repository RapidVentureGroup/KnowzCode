# KCv2.0: Retrofit Existing Project

## Context & Prerequisites

You are retrofitting KnowzCode to an existing codebase. The user has:

> **Automation Path:** Use `/kc-plan plan=project-generator` followed by `/kc-step phase=1A` with the `retrofit-specialist` subagent to capture the legacy footprint before running this prompt.
- ✅ **Downloaded KnowzCode ZIP** and extracted to project root
- ✅ **Existing codebase** with working application  
- ✅ **KnowzCode template files** present in `knowzcode/`, `knowzcode/specs/`, `knowzcode/prompts/`
- ✅ **Template `knowzcode/environment_context.md`** present and needs completion

## Your Mission

Perform comprehensive analysis of the existing project and **update the KnowzCode template files** to represent the actual codebase. The goal is complete "as-is" documentation of existing components, ready for systematic KnowzCode development.

**CRITICAL RULE: This is analysis and documentation ONLY. You MUST NOT modify application source code. Only update KnowzCode template files and create specifications.**
...
1. **Count remaining brackets**: 
   ```bash
   grep -c "\[.*\]" knowzcode/environment_context.md
   ```
...
- Review `KCv2.0__Architecture_Generator.md` for proper NodeID patterns
...
### Phase 2: Update All Core KnowzCode Files

#### 2.1 Update `knowzcode/knowzcode_project.md`
...
#### 2.2 Update `knowzcode/knowzcode_architecture.md` - Following Architecture Generator Conventions
...
- Consult `KCv2.0__Architecture_Generator.md` for exact conventions
...
**Populate `knowzcode/knowzcode_tracker.md` with every NodeID:**
...
total_nodeids=$(grep -o "[A-Z][A-Z_]*[A-Z]" knowzcode/knowzcode_architecture.md | sort -u | wc -l)
...
```bash
# Extract all NodeIDs from the completed architecture
nodeid_list=$(grep -o "[A-Z][A-Z_]*[A-Z]" knowzcode/knowzcode_architecture.md | sort -u)
...
sed -i "s/Progress: [0-9]*%/Progress: $progress_percent%/" knowzcode/knowzcode_tracker.md
...
**Prepend comprehensive entry to `knowzcode/knowzcode_log.md`:**
...
**Logged By:** AI-Agent
**Details:**
Successfully retrofitted KnowzCode v2.0 framework onto existing project.
...
**Commit all KnowzCode files to version control:**

```bash
# Stage all KnowzCode framework files
git add knowzcode/ specs/ planning/ prompts/ environment_context.md

# Create comprehensive commit
git commit -m "feat: Complete KnowzCode v2.0 retrofit with existing component documentation
...
- Ready for systematic KnowzCode development methodology"
```
...
if [ $bracket_count -eq 0 ]; then
    ...
    echo "❌ FAILED: $bracket_count placeholders still remain in knowzcode/environment_context.md"
...
if grep -q "local_dev_preview" knowzcode/environment_context.md && grep -q "public_deployed_app" knowzcode/environment_context.md; then
    ...
    if grep -q "DO NOT.*test\|real users\|DO NOT MODIFY" knowzcode/environment_context.md; then
...
invalid_nodeids=$(grep -oE '\b[A-Z][A-Z_]*[A-Za-z]+\b' knowzcode/knowzcode_architecture.md | grep -v "^L_" | grep -vE "^[A-Z]+_[A-Za-z]+")
...
- **No [bracketed placeholders]** remain in ANY KnowzCode files
...
- [ ] **Version control**: All KnowzCode files committed with descriptive message
...
**Next Command**: Use `KCv2.0__Onboarding_Audit_Verification.md` to verify retrofit completeness and identify highest priority development tasks.

**After Audit Passes**: Use `KCv2.0__Start_Work_Session.md` to begin systematic development.
