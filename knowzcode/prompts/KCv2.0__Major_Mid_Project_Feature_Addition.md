# KnowzCode v2.0: Major Mid-Project Feature Addition

**New Feature Requirement:**
[Orchestrator: Provide a detailed description of the new feature or set of capabilities to be added to the project.]

> **Automation Path:** Execute `/kc-plan plan=major-feature` so the `planning-strategist` and `impact-analyst` subagents can preload dependencies before you commit to this scope.

---

## Your Mission
You have been tasked with integrating a new, major feature into an active project. This is a high-level planning and architectural task. Your mission is to analyze the new requirement, design its integration into the existing system, update all KnowzCode project artifacts to reflect the new scope, and provide a clear, prioritized plan for implementation.

**CRITICAL RULE: This prompt is for planning and documentation updates ONLY. You MUST NOT write or modify any application source code (e.g., `.js`, `.py`, `.html` files). The implementation will be handled by subsequent, standard KnowzCode loops.**

---

### Phase 1: Deep Impact Analysis
1.  **Analyze Current State:**
    *   Review `knowzcode/knowzcode_tracker.md` to understand current progress and identify stable vs. in-progress areas.
    *   Use `git log` and `knowzcode/knowzcode_log.md` to understand the most recent changes and architectural decisions.
2.  **Architectural Integration Analysis:**
    *   Thoroughly analyze the new feature requirement in the context of `knowzcode/knowzcode_architecture.md`.
...
1.  **Proposed Architectural Changes:**
    *   Provide the new Mermaid syntax block(s) that should be added to `knowzcode/knowzcode_architecture.md`. Use subgraphs to clearly delineate the new feature.
...
3.  **Recalculate and Update Progress:**
    *   Update the `%% Progress: X% %%` line in `knowzcode_tracker.md` to reflect the new total number of nodes. Acknowledge that this percentage will decrease.

### Phase 4: Log the Expansion Operation
*   Prepend a single, comprehensive `FeatureAddition` entry to `knowzcode_log.md`. The entry **MUST** use the following format and an environment-sourced timestamp:
    ```markdown
    ---
    ...
    **Details:**
    ...
    - **Architectural Impact:** [Briefly describe the changes made to `knowzcode_architecture.md`].
...
**Example Response:**
> "The new feature '[Feature Name]' has been fully integrated into the KnowzCode planning and tracking artifacts.
> 
> *   `knowzcode_architecture.md` has been updated with [X] new components.
> *   `knowzcode_tracker.md` has been updated with [X] new `[TODO]` tasks, and the project progress is now [New %]%.
> *   A `FeatureAddition` event has been logged.
> 
> The project is now ready for implementation of the new feature.
> 
> **Recommendation:** To begin, initiate the KnowzCode loop for the first component in the implementation plan.
> **Next Command:** Use `KnowzCode v2.0: Start Work Session` and provide the `PrimaryGoal`: "Implement `[First_NodeID_from_plan]`"."
