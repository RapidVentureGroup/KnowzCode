# KCv2.0__Post_Installation_Audit.md
...
> **Automation Path:** Execute `/kc-audit audit=architecture` to leverage the `architecture-reviewer` subagent while running this checklist.

---

## Instructions for AI Agent

You are conducting a critical verification audit after the `KCv2.0__Install_And_Reconcile.md` process. Your role is to **verify Install kept its promises**, not to redo its work.
...
- All core KnowzCode files must exist
...
bracket_count=$(grep -c "\[.*\]" knowzcode/environment_context.md)
...
if grep -q "local_dev_preview" knowzcode/environment_context.md && grep -q "public_deployed_app" knowzcode/environment_context.md; then
    echo "✅ PASS: Both development and production URLs documented"
    
    # Extract and display the URLs
    dev_url=$(grep -A 2 'local_dev_preview:' knowzcode/environment_context.md | grep 'url:' | head -1 | cut -d'"' -f2)
    prod_url=$(grep -A 2 'public_deployed_app:' knowzcode/environment_context.md | grep 'url:' | head -1 | cut -d'"' -f2)
    
    echo "Development URL: $dev_url"
    echo "Production URL: $prod_url"
    
    # Verify clear usage instructions
    if grep -q "DO NOT.*test\|Primary development testing URL" knowzcode/environment_context.md; then
...
nodeid_count=$(grep -o "[A-Z][A-Z_]*[A-Z]" knowzcode/knowzcode_architecture.md | sort -u | wc -l)

# Count spec files
spec_count=$(ls knowzcode/specs/*.md 2>/dev/null | wc -l)
...
if grep -q "MVP Implementation Status\|MVP Completion" knowzcode/knowzcode_project.md; then
...
mvp_missing_count=$(grep -c "Required for MVP\|PLANNED.*MVP" knowzcode/knowzcode_tracker.md)
echo "Missing MVP components identified: $mvp_missing_count"
```

**PROMISE 5: All Core Files Populated**
- [ ] `knowzcode/knowzcode_project.md` - No [placeholders], real tech stack versions, **MVP analysis included**
- [ ] `knowzcode/knowzcode_architecture.md` - Valid Mermaid diagram with **all NodeIDs (existing + missing)**
- [ ] `knowzcode/knowzcode_tracker.md` - **All NodeIDs listed** (existing + missing for MVP) with realistic statuses
- [ ] `knowzcode/knowzcode_log.md` - SystemInitialization entry with **complete system summary** and **environment distinction**
...
if grep -q "subgraph Legend" knowzcode/knowzcode_architecture.md; then
...
nodeids=$(grep -oE '\b[A-Z][A-Z_]*[A-Za-z]+\b' knowzcode/knowzcode_architecture.md | grep -v "^L_" | sort -u)
invalid_count=0
...
ui_count=$(grep -c "UI_[A-Za-z]*\[/" knowzcode/knowzcode_architecture.md)
ui_wrong=$(grep -c "UI_[A-Za-z]*\[^/" knowzcode/knowzcode_architecture.md)
...
if grep -q "L_UI\[/.*\]" knowzcode/knowzcode_architecture.md; then
...
if grep -q "Key Features (In Scope for MVP)" knowzcode/knowzcode_project.md; then
    echo "✅ MVP features section found" >> mvp_analysis_log.txt
    
    # For each MVP feature, verify analysis was done
    mvp_features=$(grep -A 10 "Key Features (In Scope for MVP)" knowzcode/knowzcode_project.md | grep -E "^\s*\*.*:" | wc -l)
...
planned_components=$(grep -c "PLANNED.*MVP\|Required for MVP\|Missing.*MVP" knowzcode/knowzcode_tracker.md)
...
if grep -q "environment_focus.*DEVELOPMENT\|Environment Type.*DEVELOPMENT" knowzcode/environment_context.md; then
...
if grep -q "DO NOT.*test.*production\|DO NOT USE FOR TESTING" knowzcode/environment_context.md; then
...
if grep -q "Development URL\|local_dev_preview\|Environment Focus" knowzcode/knowzcode_log.md; then
...
find . -name "*.jsx" -o -name "*.tsx" -o -name "*.vue" -o -name "*.svelte" | grep -v node_modules | while read file; do
    ...
    if ! grep -q "UI_$component_name\|$component_name" knowzcode/knowzcode_architecture.md; then
...
find . -name "*model*" -o -name "*schema*" -o -name "*migration*" | grep -v node_modules | while read file; do
    component_name=$(basename "$file" | sed 's/\.[^.]*$//')
    if ! grep -q "MODEL_\|DATA_\|MIGRATION_" knowzcode/knowzcode_architecture.md; then
...
find . -name "*util*" -o -name "*helper*" -o -name "*config*" -o -name "constants*" | grep -v node_modules | while read file; do
    component_name=$(basename "$file" | sed 's/\.[^.]*$//')
    if ! grep -q "UTIL_\|CONFIG_\|CONST_" knowzcode/knowzcode_architecture.md; then
...
for nodeid in "${missing_nodeids[@]}"; do
    ...
    # Add to knowzcode/knowzcode_architecture.md in appropriate subgraph
    
    echo "Adding $nodeid to tracker..." >> gap_analysis_log.txt  
    # Add to knowzcode/knowzcode_tracker.md with appropriate status
done
...
final_nodeid_count=$(grep -o "[A-Z][A-Z_]*[A-Z]" knowzcode/knowzcode_architecture.md | sort -u | wc -l)
...
existing_count=$(grep -c "✅.*IMPLEMENTED\|🟢.*VERIFIED" knowzcode/knowzcode_tracker.md)
planned_count=$(grep -c "⚪.*PLANNED\|Required for MVP" knowzcode/knowzcode_tracker.md)
todo_existing=$(grep -c "⚪.*TODO" knowzcode/knowzcode_tracker.md)
issue_count=$(grep -c "❗.*ISSUE" knowzcode/knowzcode_tracker.md)
...
verified_count=$(grep -c "🟢 \[VERIFIED\]" knowzcode/knowzcode_tracker.md)
todo_count=$(grep -c "⚪ \[TODO\]" knowzcode/knowzcode_tracker.md)
planned_count=$(grep -c "⚪.*PLANNED" knowzcode/knowzcode_tracker.md)
issue_count=$(grep -c "❗ \[ISSUE\]" knowzcode/knowzcode_tracker.md)
...
if grep -q "npm run dev\|yarn dev\|python.*runserver" knowzcode/environment_context.md; then
...
if grep -q "Primary development testing URL\|Use.*for.*testing" knowzcode/environment_context.md; then
...
- **Ready**: For systematic KnowzCode development methodology toward MVP completion
...
Add audit completion entry to `knowzcode/knowzcode_log.md`:
...
**Next Command**: Use `KCv2.0__Start_Work_Session.md` to begin systematic development toward MVP completion
```
...
**Recommended**: Address identified issues, then use `KCv2.0__Start_Work_Session.md`
```
