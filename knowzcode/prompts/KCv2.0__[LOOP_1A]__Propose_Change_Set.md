# KnowzCode v2.0: [LOOP 1A] Propose Change Set
...
> **Automation Path:** Run `/kc-step phase=1A` to engage the `impact-analyst` subagent with the `load-core-context`, `tracker-scan`, and `generate-workgroup-id` skills preloaded. Review the summary in `knowzcode/automation_manifest.md` if you need a refresher on available helpers.
...
*   You will now **PAUSE** and await the Orchestrator's next command, which will be **`KCv2.0__[LOOP_1B]__Draft_Specs.md`**. Do not take any further action.
